...
#================================================================================
#   
#    hcdiag/src/tests/hcatest/hcatest.sh
# 
...
do
    ...
    do
        ...
        xdsh $server -t 1 "/usr/bin/ib_send_lat -d mlx5_$idx -p $port &>/dev/null &" &>/dev/null
        xdsh $client "/usr/bin/ib_send_lat -d mlx5_$idx -p $port $server 2>&1" &>>/tmp/$$
    done
    echo -e "\n\n" >>/tmp/$$
done
...
echo -e "Unable to connect to $server for ib_send_bw"
echo -e "\n$(basename $0) test FAIL, rc=1"
exit 1
