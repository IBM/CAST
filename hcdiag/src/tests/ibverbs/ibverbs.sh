...
#================================================================================
#   
#    hcdiag/src/tests/ibverbs/ibverbs.sh
# 
...
do
    ...
    do
        ...
        xdsh $server -t 1 "/usr/bin/ib_send_bw -b -d mlx5_$idx -p $port &>/dev/null &" &>/dev/null
        xdsh $client "/usr/bin/ib_send_bw -b -d mlx5_$idx -p $port $server 2>&1" &>>/tmp/$$
    done
    echo -e "\n\n" >>/tmp/$$
done
