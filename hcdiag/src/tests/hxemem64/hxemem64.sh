...
#================================================================================
#   
#    hcdiag/src/tests/hxemem64/hxemem64.sh
# 
...
rule="mem:mem.coral.pass$pass"
thismdt="${htxd_root}/mdt/mdt.hxemem64.coral_pass${pass}"
get_processor
...
if [ $rc -eq 0 ]; then 
   # creates the mdt file to run hxemem64
   htxdiag_get_mdt $rule $thismdt 
   rc=$ret
   if [ $rc -eq 0 ]; then 
      # check if we need to patch the mdt file 
      if [ $processor == "p8" ]; then 
         s=`grep 'HE_name = "hxemem64"' $thismdt`
         if [ -n "$s" ]; then
            cat $thismdt | sed 's/HE_name = "hxemem64"/HE_name = "hxemem64_new"/' > /tmp/$$
            sudo mv /tmp/$$ $thismdt
         fi
      fi
