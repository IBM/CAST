...
ls /etc/ibm/csm/csm_ufm_ssl_key.txt > ${TEMP_LOG} 2>&1
check_return_flag_value $? 2 "Test Case 1: standalone_ib_and_switch_collection fails without SSL key - remove SSL key"
${SBIN_PATH}/standalone_ib_and_switch_collection -c /etc/ibm/csm/csm_master/cfg -t 3 > ${TEMP_LOG} 2>&1
check_return_flag_value $? 1 "Test Case 1: standalone_ib_and_switch_collection fails without SSL key"
cp -f ${SSL_KEY} /etc/ibm/csm/csm_ufm_ssl_key.txt > ${TEMP_LOG} 2>&1
ls /etc/ibm/csm/csm_ufm_ssl_key.txt > ${TEMP_LOG} 2>&1
check_return_flag_value $? 0 "Test Case 1: standalone_ib_and_switch_collection fails without SSL key - restore SSL key"
...
check_return_flag_value $? 0 "Test Case 2: standalone_ib_and_switch_collection collects IB cable data"
...
check_return_flag_value $? 0 "Test Case 3: Checking IB cable data populated"
...
check_return_flag_value $? 0 "Test Case 4: standalone_ib_and_switch_collection collects Switch data"
...
check_return_flag_value $? 0 "Test Case 5: Checking Switch data populated"
...
${SBIN_PATH}/standalone_ib_and_switch_collection -c /etc/ibm/csm/csm_master.cfg -t 3 > ${TEMP_LOG} 2>&1
check_return_flag_value $? 0 "Test Case 6: standalone_ib_and_switch_collection collects IB and Switch data"
...
check_return_flag_value $? 0 "Test Case 7: Checking IB cable data populated"

# Test Case 8: Checking Switch data populated
su -c "psql -d csmdb -c 'SELECT * FROM csm_switch LIMIT 1;'" postgres | grep "1 row" > ${TEMP_LOG}
check_return_flag_value $? 0 "Test Case 8: Checking Switch data populated"
