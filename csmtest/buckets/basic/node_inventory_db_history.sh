...
# Initialize DB table lookup arrays
declare -A csm_node_history_column_name_to_column_index
declare -A csm_ssd_history_column_name_to_column_index
declare -A csm_hca_history_column_name_to_column_index
declare -A csm_gpu_history_column_name_to_column_index
declare -A csm_dimm_history_column_name_to_column_index
declare -A csm_processor_socket_history_column_name_to_column_index

for i in $(/bin/psql -U postgres csmdb -c "select * from csm_node_history limit 0;" | head -n 1 | tr "|" "\n")
do
   echo "$i"
done
...
# Clean table data
su -c "/bin/psql -d csmdb -c 'DELETE FROM csm_ib_cable ;'" postgres
su -c "/bin/psql -d csmdb -c 'DELETE FROM csm_switch ;'" postgres 

# Test Case 1: standalone_ib_and_switch_collection collects IB cable data
echo "Test Case 1: standalone_ib_and_switch_collection collects IB cable data" >> ${LOG}
${SBIN_PATH}/standalone_ib_and_switch_collection /etc/ibm/csm/csm_master.cfg 1>> ${TEMP_LOG} 2>&1
check_return_exit $? 0
echo "Checking IB cable data populated..." >> ${LOG}
su -c "/bin/psql -d csmdb -c 'SELECT * FROM csm_ib_cable LIMIT 1;'" postgres | grep "1 row"
check_return_flag $? "Test Case 1: standalone_ib_and_switch_collection did not populate IB cable data"

rm -f ${TEMP_LOG}
su -c "/bin/psql -d csmdb -c 'DELETE FROM csm_ib_cable ;'" postgres

echo "------------------------------------------------------------" >> ${LOG}
# Test Case 2: standalone_ib_and_switch_collection collects Switch data
echo "Test Case 2: standalone_ib_and_switch_collection collects Switch data..." >> ${LOG}
${SBIN_PATH}/standalone_ib_and_switch_collection /etc/ibm/csm/csm_master.cfg 2 > ${TEMP_LOG} 2>&1
check_return_exit $? 0
echo "Checking Switch data populated..." >> ${LOG}
su -c "/bin/psql -d csmdb -c 'SELECT * FROM csm_switch LIMIT 1;'" postgres | grep "1 row"
check_return_flag $? "Test Case 2: standalone_ib_and_switch_collection did not populate Switch data"

rm -f ${TEMP_LOG}
su -c "/bin/psql -d csmdb -c 'DELETE FROM csm_switch ;'" postgres

echo "------------------------------------------------------------" >> ${LOG}
# Test Case 3: standalone_ib_and_switch_collection collects IB and Switch data
echo "Test Case 3: standalone_ib_and_switch_collection collects IB and Switch data..." >> ${LOG}
${SBIN_PATH}/standalone_ib_and_switch_collection /etc/ibm/csm/csm_master.cfg 3 > ${TEMP_LOG} 2>&1
check_return_exit $? 0
echo "Checking IB cable data populated..." >> ${LOG}
su -c "/bin/psql -d csmdb -c 'SELECT * FROM csm_ib_cable LIMIT 1;'" postgres | grep "1 row"
check_return_flag $? "Test Case 3: standalone_ib_and_switch_collection did not populate IB cable data"
echo "Checking Switch data populated..." >> ${LOG}
su -c "/bin/psql -d csmdb -c 'SELECT * FROM csm_switch LIMIT 1;'" postgres | grep "1 row"
check_return_flag $? "Test Case 3: standalone_ib_and_switch_collection did not populate Switch data"
