    ...
    ALLOC_STRUCT **output )
{
    ...
    if ( !( fields->nfields == 30 || fields->nfields == 33 ) ) 
    {
        *output = nullptr;
        return false;
    }
    ...
    if ( fields->data[30][0] )
    {
        ...
        o->history->end_time             = strdup(fields->data[30]);
        o->history->exit_status          = strtol(fields->data[31], nullptr, 10);
        o->history->archive_history_time = strdup(fields->data[32]);
    }
