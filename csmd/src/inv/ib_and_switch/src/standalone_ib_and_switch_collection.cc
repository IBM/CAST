...
void help(){
        ...
        puts("USAGE:");
        puts("  inventory_collection ARGUMENTS [OPTIONS]");
        puts("  inventory_collection [-h] [-v verbose_level]");
        puts("");
        puts("SUMMARY: Used to collect csm switch and ib inventory and insert into CSM database.");
        puts("");
        puts("EXIT STATUS:");
        puts("  0  if OK,");
        puts("  1  if ERROR.");
...
int main(int argc, char *argv[])
{
 ...
 // printing
 std::cout << "The first parameter should be the full path to the master configure file" << std::endl;
 std::cout << "The second parameter should be the type of data collection (type_of_collection) that you want to execute" << std::endl;
 std::cout << std::endl;

 // printing
 std::cout << "Type_of_collection 1 -> only ib cables" << std::endl;
 std::cout << "Type_of_collection 2 -> only switches" << std::endl;
 std::cout << "Type_of_collection 3 -> ib cables and switches" << std::endl;
 std::cout << std::endl;
 ...
 if ( argc != 3 )
 {
  ...
  // printing
  std::cout << "You should give 2 input arguments" << std::endl;
 ...
 std::string full_path_to_the_master_config_file = argv[1];
 std::cout << "full path to the master config file: " << full_path_to_the_master_config_file << std::endl;

 // type of collection
 long int type_of_collection = std::atol(argv[2]);
 ...
 if ( type_of_collection == 1 )
 {
  ...
  if ( ib_flag == 0 )
  {
   ...
   INV_IB_CONNECTOR_ACCESS::GetInstance()->ExecuteDataCollection(rest_address,authentication_string_for_the_http_request);
 ...
 //std::cout << "Test for switch field value " << switch_field_value << std::endl;
        ...
        */
        ...
        {
         csmutil_logging(error, "csm_term_lib rc= %d, Initalization failed. Success "
