...
#undef PFSBLOCKBOUNDRY
        ...
        if (!rc)
        {
            ...
            if (!rc)
            {
                ...
                for(std::vector<Extent>::size_type i = 0; i < pTransferDef->extents.size(); ++i)
                {
                    allExtents.push_back(ExtentInfo(pHandle, pContribId, &(pTransferDef->extents[i]), pTransferDef));
                }
            }
            else
            {
...
int BBLVKey_ExtentInfo::setSuspended(const LVKey* pLVKey, const string& pHostName, const uint64_t pJobId, const int pValue)
{
    int rc = 0;
