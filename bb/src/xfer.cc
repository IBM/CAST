...
int contribIdStopped(const std::string& pConnectionName, const LVKey* pLVKey, BBTagInfo2* pTagInfo2, BBTransferDef* pOrigTransferDef, BBTransferDef* pRebuiltTransferDef, const uint64_t pJobId, const uint64_t pJobStepId, const uint64_t pHandle, const uint32_t pContribId)
{
    ...
    while (!l_AllDone)
    {
        ...
        while ((rc != 1) && (l_Continue--))
        {
            ...
            if (rc >= 0)
            {
                if (rc == 1 && l_ContribIdFile)
                {
                    if (l_ContribIdFile->allExtentsTransferred())
                    {
                        ...
                        {
                            // All extents have been transferred but the transfer definition is not marked as stopped
                            rc = 0;
                            l_Continue = 0;
                            LOG(bb,info) << "contribIdStopped(): All extents have been transferred for contribId " << pContribId \
                                         << ", but it is not marked as being stopped.   All transfers for this contributor have already finished." \
                                         << " See previous messages.";
                        }
                    }
                    else
                    {
                        // Not all previously enqueud extents have been processed
            ...
            if ((!rc) && l_Continue)
            {
                // Spin, waiting for the transfer definiion to be marked as stopped and for all of the prior enqueued
                ...
                if ((i++ % 40) == 4)
                {
                    LOG(bb,info) << ">>>>> DELAY <<<<< contribIdStopped(): Attempting to restart a transfer definition for jobid " << pJobId \
                                 ...
                                 << ". Waiting for all prior enqueued extents to finish being processed from the work queue. Delay of 250 milliseconds.";
        ...
        if (rc == -2)
        {
            if (++l_Attempts <= 2)
            {
                rc = prepareForRestartOriginalServerDead(pConnectionName, pLVKey, pHandle, BBJob(pJobId, pJobStepId), pContribId, pOrigTransferDef, pRebuiltTransferDef);
                if (!rc)
                {
...
int prepareForRestartOriginalServerDead(const std::string& pConnectionName, const LVKey* pLVKey, const uint64_t pHandle, BBJob pJob, const int32_t pContribId,
                                        BBTransferDef* pOrigTransferDef, BBTransferDef* pRebuiltTransferDef)
{
    int rc = 0;

    stringstream l_JobStr;
    pJob.getStr(l_JobStr);
    LOG(bb,info) << "prepareForRestartOriginalServerDead(): Entering for " << *pLVKey << ", " << l_JobStr \
                 << ", handle " << pHandle << ", contribid " << pContribId;

    // The following contain the parts of the three passes for prepareForRestart() to reset the cross bbServer
    // metadata for the input transfer definition that would have been called on the original bbServer,
    // if it was responding.

    // If the transfer definition is passed (the one that would now be addressed in BBTagInfo/BBTagParts),
    // perform that cleanup...
    if (pOrigTransferDef)
    {
        rc = pOrigTransferDef->prepareForRestart(pLVKey, pJob, pHandle, pContribId, pRebuiltTransferDef, SECOND_PASS);
        if (!rc)
        {
            rc = pOrigTransferDef->prepareForRestart(pLVKey, pJob, pHandle, pContribId, pRebuiltTransferDef, THIRD_PASS);
        }
        else
        {
            LOG(bb,error) << "prepareForRestartOriginalServerDead(): Error from prepareForRestart() for original transfer definition, second pass.";
        }
    }
    ...
    if (!rc)
    {
        ...
        if (!rc)
        {
            // Reset the flags for the HandleFile...
        ...
        {
            LOG(bb,error) << "prepareForRestartOriginalServerDead(): Error from update_xbbServerFileStatusForRestart()";
        }
    }
    else
    {
        LOG(bb,error) << "prepareForRestartOriginalServerDead(): Error from prepareForRestart() for original transfer definition, third pass.";
    }

    LOG(bb,info) << "prepareForRestartOriginalServerDead(): Exiting for " << *pLVKey << ", " << l_JobStr \
                 << ", handle " << pHandle << ", contribid " << pContribId;
            ...
            if (!rc)
            {
                ...
                if (!pPerformOperation)
                {
                    if (!l_OrigTransferDef)
                    {
                        ...
                        // NOTE:   transfer definition from the retrieve archive for a restart transfer definition scanario.
                        // NOTE: * addTransferDef() resets pTransferDef to a version of the transfer definition in BBTagInfo/BBTagParts.
            ...
            if (!pMarkFailedFromProxy)
            {
                if (pTransferDef)
                {
                    if (pPerformOperation)
                    {
                        ...
                        if (l_TagInfo)
                        {
                            BBTransferDef* l_TransferDef = l_TagInfo->getTransferDef((uint32_t)pContribId);
                            if (l_TransferDef)
                            {
                                ...
                                if (!rc)
                                {
                                    ...
                                    if (!rc)
                                    {
                                        ...
                                        if (!wrkqmgr.getWrkQE(pLVKey, l_WrkQE))
                                        {
                                            ...
                                            {
                                                // Inconsistency with metadata....
                                                errorText << "Work queue entry was not returned from work queue manager when attempting to schedule additional extents to transfer";
                                                LOG_ERROR_TEXT(errorText);
                                                rc = -1;
                                            }
                                        }
                                        ...
                                        {
                                            // Inconsistency with metadata....
                                            errorText << "Could not find work queue when attempting to schedule additional extents to transfer";
                                            LOG_ERROR_TEXT(errorText);
                                            rc = -1;
                                        }
                                    ...
                                    {
                                        // \todo - Handle error?
                                        LOG(bb,error) << "sortExtents() failed, rc = " << rc;
                                    }
                                }
                                else
                                {
                                    // \todo - Handle error?
                                    LOG(bb,error) << "addExtents() failed, rc = " << rc;
                                }
                            }
                            else
                            {
                                // Should never happen...  \todo - Error?  @DLH
                            }
                        }
                        else
                        {
