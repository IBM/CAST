                                                 ...
                                                 const ALLOW_BUMP_FOR_REPORTING_CONTRIBS_OPTION pAllowBumpOfReportingContribs, const HANDLEFILE_LOCK_OPTION pLockOption, const uint64_t pFlags, const int pValue)
{
    ...
    else
    {
        ...
        LOG(bb,error) << "Could not lock the handle file for " << *pLVKey << ", jobid " << pJobId << ", jobstepid " << pJobStepId << ", handle " << pHandle \
                      << ", contribid " << pContribId << " when attempting to update the contribid file";
    }
...
int ContribIdFile::update_xbbServerFileStatus(const LVKey* pLVKey, BBTransferDef* pTransferDef, uint64_t pHandle, uint32_t pContribId, Extent* pExtent, const uint64_t pFlags, const int pValue)
{
    ...
    if (pTransferDef->stopped())
    {
        // NOTE: The handle file is locked exclusive here to serialize between this bbServer and another
        //       bbServer that is attempting to restart this transfer definition.
        rc = HandleFile::loadHandleFile(l_HandleFile, l_HandleFileName, pTransferDef->getJobId(), pTransferDef->getJobStepId(), pHandle, LOCK_HANDLEFILE, &l_LockFeedback);
    }
        ...
        switch (rc)
        {
            case 1:
            {
                ...
                if ((pExtent->sourceindex%2) == 0)
                {
                    ...
                    {
                        LOG(bb,debug) << "update_xbbServerFileStatus(): Current AET: " << hex << uppercase << (l_NewContribIdFlags & BBTD_All_Extents_Transferred) << nouppercase << dec \
                                      << ", Input flags: " << pFlags << ", allFilesClosed(): " << l_ContribIdFile->allFilesClosed();
            ...
            default:
            {
                LOG(bb,error) << "Could not load the contribid file for " << *pLVKey << ", jobid " << pTransferDef->getJobId() << ", jobstepid " << pTransferDef->getJobStepId() << ", handle " << pHandle << ", contribid " << pContribId << ", using handle path " << l_HandleFilePath.string();
            }
        }
    }
    else
    {
        ...
        LOG(bb,error) << "ContribIdFile::update_xbbServerFileStatus(): Could not lock the handle file for " << *pLVKey << ", jobid " << pTransferDef->getJobId() << ", jobstepid " << pTransferDef->getJobStepId() \
                      << ", handle " << pHandle << ", contribid " << pContribId << " when attempting to update the contribid file status";
    }
