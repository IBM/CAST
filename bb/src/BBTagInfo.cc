    ...
    if (pPass == FIRST_PASS)
    {
        ...
        while (!l_AllDone)
        {
                ...
                if (rc != 2)
                {
                    if (((wrkqmgr.getDeclareServerDeadCount() - l_Continue) % 15) == 1)
                    {
                        ...
                        LOG(bb,info) << ">>>>> DELAY <<<<< BBTagInfo::prepareForRestart: Attempting to restart a transfer definition for jobid " << pJob.getJobId() \
                                     ...
                                     << ". Waiting for the handle to be marked as stopped. Delay of 1 second. " << l_Continue \
            ...
            if (rc > 0)
            {
                if (++l_Attempts <= 2)
                {
                    rc = prepareForRestartOriginalServerDead(pConnectionName, pLVKey, pHandle, pJob, pContribId, l_OrigTransferDef, pRebuiltTransferDef);
                    if (!rc)
                    {
                    ...
                    else
                    {
                        // Indicate not to restart this transfer definition
                        rc = 1;
                        LOG(bb,info) << "Attempt to reset the cross bbServer metadata for the transfer definition associated with " << *pLVKey << ", jobid " << pJob.getJobId() \
                                     << ", jobstepid " << pJob.getJobStepId() << ", handle " << pHandle << " to a stopped state failed. Cannot restart any transfer definitions under this handle.";
                    }
                }
                else
                {
                    // Indicate not to restart this transfer definition
                    rc = 1;
                    LOG(bb,info) << *pLVKey << ", jobid " << pJob.getJobId() << ", jobstepid " << pJob.getJobStepId() \
