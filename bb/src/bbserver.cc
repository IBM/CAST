...

        {
            ...
            switch (l_CancelScope)
            {
                case BBSCOPETRANSFER:
                {
                    ...
                    {
                        l_TagInfo2 = metadata.getTagInfo2(l_LVKey);
                        if (!l_TagInfo2)
                        {
                    ...
                    if (!l_TagInfo2)
                    {
                        errorText << "Could not resolve to BBTagInfo2 for " << *l_LVKey << ", jobid " << l_FromJobId << ", contribid " << l_ContribId;
                        rc = -1;
                        LOG_ERROR_TEXT_RC_AND_BAIL(errorText, rc);
                    }
                    ...
                    {
                        if (!l_TagInfo2->getTagInfo(l_Handle, l_ContribId, l_TagId, l_TagInfo))
                        {
                    ...
                    if (l_TagInfo2->getExtentInfo()->moreExtentsToTransfer(l_Handle, l_ContribId, 0))
                    {
    ...
    try
    {
        ...
        while ((!rc) && (l_Continue--))
        {
            ...
            if (rc >= 0)
            {
                ...
                // rc=1 incidate that an existing LVKey was found
            ...
            else
            {
                ...
                if (rc == -2)
                {
                    ...
                    if (rc2 == 1)
                    {
                        ...
                        // is started using this handle.  The necessary LVKey will be added
                        // at start transfer time...
                        rc = 0;
                        l_Continue = 0;
                    }
                    else
                    {
                        if (!l_Continue)
                        {
                            // We have waited long enough... (30 seconds total...)
                        ...
                        {
                            rc = 0;
                            // Hang out for a bit (60 x 500 mils each) and see if the necessary LVKey appears...
...
void msgin_starttransfer(txp::Id id, const string& pConnectionName, txp::Msg* msg)
{
    ...
    try
    {
...

        {
            ...
            while ((!rc) && (!l_AllDone))
            {
                rc = metadata.getInfo(pConnectionName, l_LVKey2, l_TagInfo2, l_TagInfo, l_TagId, l_Job, l_Contrib, l_Handle, l_ContribId);

                // NOTE: If this is for a restart and the getInfo() above was successful, then the wait to make sure all prior enqueued
                //       extents have been processed by the previous bbServer is done by the contribIdStopped() processing that is
                //       invoked via the first pass of prepareForRestart(), which is invoked downstream of queueTransfer().
                //       Otherwise, if the getInfo() above was not successful (i.e., the logical volume has not yet been registered to this
                //       bbserver), then the wait for the LVKey to be registered is done by the code in the else leg below.
                if (rc > 0)
                {
                    ...
                    // NOTE:  The check for the suspend state is only done if we are not performing the operation (first volley from bbProxy).
                    //        Once the local metadata is primed for this start transfer operation, we must continue forward.  Otherwise,
                    //        a remove logical volume would first be required before the transfer could be started.  This is essentially the
                    //        case when a suspend occurs between the two message volleys for a start transfer.
                    if (!((!l_PerformOperation) && l_TagInfo2->isSuspended()))
                    {
                        // Not suspended...
                        ...
                        int l_Attempts = 0;
                        rc = -1;
                        while (rc && ++l_Attempts < 120)
                        {
                            rc = wrkqmgr.getWrkQE(&l_LVKey2, l_WrkQE);
                            if (rc)
                            {
                                unlockTransferQueue(&l_LVKey, "msgin_starttransfer (restart) - Waiting for LVKey's work queue");
                                {
                                    usleep((useconds_t)250);    // Delay 250 milliseconds
                                }
                                lockTransferQueue(&l_LVKey, "msgin_starttransfer (restart) - Waiting for LVKey's work queue");
                            }
                        }
                        if (!rc)
                        {
                ...
                else
                {
                    if (!l_PerformOperation)
                    {
                        ...
                        rc = HandleFile::loadHandleFile(l_HandleFile, l_HandleFileName, l_Job.getJobId(), l_Job.getJobStepId(), l_Handle);
                        if (!rc)
                        {
                            ...
                            if (hasContribId(l_ContribId, l_NumContrib, l_ContribArray))
                            {
                                ...
                                if (l_TransferPtr->builtViaRetrieveTransferDefinition())
                                {
                                    ...
                                    while (!l_AllDone)
                                    {
                                            ...
                                            if (rc == 1)
                                            {
                                                if (l_ContribIdFile)
                                                {
                                                    if (!l_ContribIdFile->stopped())
                                                    {
                                                        ...
                                                        {
                                                            rc = 0;     //  Assume we continue to spin...
                                                            if (!l_Continue)
                                                            {
                                                                //  We have waited long enough...  The transfer definition is still not marked as stopped...
                                                                rc = -2;
                                                            }
                                                        }
                                                        else
                                                        {
                                                            // The contributor either finished or was canceled...  Feed that back to bbProxy...
                                                            l_Attempts = 2;
                                                            rc = -2;
                                                        }
                                                    }
                                        ...
                                        if (rc == -2)
                                        {
                                            if (++l_Attempts <= 2)
                                            {
                                                rc = prepareForRestartOriginalServerDead(pConnectionName, &l_LVKey, l_Handle, l_Job, l_ContribId, (BBTransferDef*)0, l_TransferPtr);
                                                if (!rc)
                                                {
                                                ...
                                                else
                                                {
                                                    ...
                                                    errorText << "Attempt to reset the cross bbServer metadata for the transfer definition associated with contribid " << l_ContribId << " to stopped failed." \
                                                              << " Therefore, the transfer definition cannot be restarted.";
                                                    LOG_ERROR_TEXT_RC_AND_BAIL(errorText, rc);
                                                }
                                            }
                                            else
                                            {
                                                // Indicate not to restart this transfer definition
                                                rc = 1;
                                                errorText << "ContribId " << l_ContribId << " found in the cross bbServer metadata, but it does not have a status of stopped." \
                                ...
                                if (!rc)
                                {
                                    ...
                                    {
                                        // Local metadata was found.  Indicate an error, as we could not find it in above processing...
                                        // NOTE: errstate already filled in...
                                        rc = -1;
                                        errorText << "Inconsistent local metadata";
                                        LOG_ERROR_TEXT_RC_AND_BAIL(errorText, rc);
                                    }
                                    else
                                    {
                                        if (rc == -2)
                                        {
                                            // LVKey could not be found...
                                            if (l_TransferPtr->builtViaRetrieveTransferDefinition() || (!l_ContribIdFile))
                                            {
                                                // This is either a restart case to a new bbServer -or-
                                                // a start transfer to a new bbServer
                                                rc = 0;

                                                // We must add the LVKey for the logical volume to the local metadata.
                                                // We will then iterate again to add the remainder of the local
                                                // metadata with getHandle(), before we can then register
                                                // the transfer definition with queueTransfer().
                                                //
                                                // NOTE: We obtained the correct lvuuid from the loadContribIdFile
                                                //       above for the restart case.  For the start case and
                                                //       we are to perform the operation, we use the lvuuid
                                                //       passed from bbProxy.
                                                if (!l_ContribIdFile)
                                                {
                                                    l_lvuuid3 = l_lvuuid;
                                                ...
                                                LVKey l_LVKey3 = std::make_pair(pConnectionName, l_lvuuid3);
                                                LVKey* l_LVKey3Ptr = &l_LVKey3;
                                                rc = addLogicalVolume(pConnectionName, l_HostName, l_LVKey3Ptr, l_Job.getJobId(), DO_NOT_TOLERATE_ALREADY_EXISTS);
                                                if (rc)
                                                {
                                                    // NOTE:  error text should already be filled in...
                                                    char lv_uuid3_str[LENGTH_UUID_STR] = {'\0'};
                                                    l_lvuuid3.copyTo(lv_uuid3_str);
                                                    bberror << err("error.jobid", l_Job.getJobId()) << err("error.uuid",lv_uuid3_str);
                                                    LOG_RC_AND_BAIL(rc);
                                                }
                                            }
                                            else
                                            {
                                                // This is a start transfer and the contribid is already associated with
                                                // another bbServer, but not this bbServer.
                                                stringstream l_JobStr;
                                                l_Job.getStr(l_JobStr);
                                                errorText << "A logical volume (LVKey) is not currently associated with " << l_JobStr.str() \
                                                          << " on this bbServer, contribid " << l_ContribId \
                                                          << " is already known for this job to another bbServer in the cluster," \
                                                          << " and this is not a restart operation for the transfer definition." \
                                                          << " This transfer definition cannot be started on this bbServer.";
                                                LOG_ERROR_TEXT_RC_AND_BAIL(errorText, rc);
                                            }
                                        }
                                        else
                        ...
                        else
                        {
                            ...
                            {
                                rc = 1;
                                LOG(bb,info) << "msgin_starttransfer(): Error occurred when attempting to re-prime the metadata for contribid " << l_ContribId << " (Negative rc from getHandle())." \
                                             << " All transfers for this contributor may have already finished.  See previous messages.";
                                BAIL;
                            }
                            else
                            {
                                rc = -1;
                                errorText << "Transfer handle could not be found on any server";
                                LOG_ERROR_TEXT_RC_AND_BAIL(errorText, rc);
                            }
                        }
                        if (l_ContribIdFile)
