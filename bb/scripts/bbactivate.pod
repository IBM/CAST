...
Artifically changes the configuration for FVT performance tests.  Not recommended for production usage.
...
=item B<--interfacename>

Changes the interfacename used by bbactivate.  bbactivate uses the interface name to configure the NVMe over Fabrics target interface.  The default is ib0.  
