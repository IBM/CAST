...
def performBasicAnalysis(pCtx):
    ...
    for l_JobId in l_JobIds:
        ...
        for l_Server in l_Servers:
            ...
            l_TestOutput.append(
                "%sJobId %d, server %s, %d jobstep(s)%s" % (4 * " ", l_JobId, l_Server, len(l_JobStepIds), os.linesep))
            for l_JobStepId in l_JobStepIds:
                l_ConnectionHandleData = cmn.getHandlesPerConnectionForJobIdJobStepId(l_Data, l_Server, l_JobId,
                                                                                      l_JobStepId)
                l_ConnectionHandles = l_ConnectionHandleData.keys()
                l_ConnectionHandles.sort()
                l_TestOutput.append(
                    "%sJobStepId %d, %d connection(s)%s" % (6 * " ", l_JobStepId, len(l_ConnectionHandles), os.linesep))
                for l_Connection in l_ConnectionHandles:
                    ...
                    8 * " ", l_Connection, len(l_Handles), `l_Handles`, os.linesep))
            l_TestOutput.append("%s" % (os.linesep))
    ...
    for l_JobId in l_JobIds:
        ...
        for l_Server in l_Servers:
            ...
            for l_JobStepId in l_JobStepIds:
                ...
                l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["ProcessingTimes (ContribId Min/Max)"] = [None, None]
                l_Output[l_JobId][l_Server][l_JobStepId]["NotSuccessfulHandles"] = []

                # For each handle...
                l_Handles = cmn.getHandlesForServer(l_Data, l_Server)
                l_NumberOfHandles = 0
                l_HandleProcessingTimes = copy.deepcopy(l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["ProcessingTimes (ContribId Min/Max)"])
                ...
                for l_Handle in l_Handles:
                    if "JobId" in l_Data[l_Server]["Handles"][l_Handle] and "JobStepId" in l_Data[l_Server]["Handles"][l_Handle]:
                        if l_Data[l_Server]["Handles"][l_Handle]["JobId"] == l_JobId and l_Data[l_Server]["Handles"][l_Handle]["JobStepId"] == l_JobStepId:
                            ...
                            for l_Connection in l_Connections:
                                if l_Connection not in l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"]:
                                    ...
                                    l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["transferTypes"] = []
                                    l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["NumberOfContribIds"] = 0
                                    l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["TotalNumberOfFiles"] = 0
                                ...
                                l_ProcessingTimes = copy.deepcopy(l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["processingTimes (File Min/Max)"])
                                l_TotalNumberOfFiles = l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["TotalNumberOfFiles"]
                                ...
                                for l_LVUuid in l_LVUuids:
                                    ...
                                    for l_ContribId in l_ContribIds:
                                        ...
                                        l_TransferTypes = set(l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["transferTypes"])
                                ...
                                l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["processingTimes (File Min/Max)"] = copy.deepcopy(l_ProcessingTimes)
                                l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["connections"][l_Connection]["contribIds"]["TotalNumberOfFiles"] = l_TotalNumberOfFiles
                            # End of connections...
                ...
                l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["ProcessingTimes (ContribId Min/Max)"] = copy.deepcopy(l_HandleProcessingTimes)
            # End of jobstepids
                        ...
                        try:
                            if "SizeTransferred" in l_Output[l_JobId][l_Server][l_JobStepId]["handles"]:
                                if l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["SizeTransferred"]:
                                    l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["Avg SizeTransferred/Handle"] = cmn.numericFormat(round(float(l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["SizeTransferred"]) / float(l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["NumberOfHandles"]), 3))
                                    l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["SizeTransferred"] = cmn.numericFormat(l_Output[l_JobId][l_Server][l_JobStepId]["handles"]["SizeTransferred"])
                        except Exception:
                            pass
        ...
        l_PathFileName = os.path.join(pCtx["ROOTDIR"], "Analysis", `l_JobId`, "Details.txt")
        cmn.printFormattedFile(pCtx, l_PathFileName, l_Output)
        l_PathFileName = os.path.join(pCtx["ROOTDIR"], "Analysis", `l_JobId`, "Details.json")
        cmn.printFormattedFileAsJson(pCtx, l_PathFileName, l_Output)

        print("Jobid details written to %s%s" % (os.path.join(pCtx["ROOTDIR"], "Analysis", `l_JobId`), os.linesep))
