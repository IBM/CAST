...
def main(*pArgs):
    ...
    l_PathFileName = os.path.join(l_Ctx["ROOTDIR"], "BasicData.txt")
    ...
    for l_JobId in l_JobIds:
        ...
        for l_Server in l_Servers:
            ...
            l_Output[l_JobId][l_Server]["Handles"]["Connections"]["NumberOfConnections"] = 0
            ...
            for l_Handle in l_Handles:
                if l_Data[l_Server]["Handles"][l_Handle]["JobId"] == l_JobId:
                    ...
                    l_NumberOfConnections = l_Output[l_JobId][l_Server]["Handles"]["Connections"]["NumberOfConnections"]
                    for l_Connection in l_Connections:
                        ...
                        for l_LVUuid in l_LVUuids:
                            ...
                            for l_ContribId in l_ContribIds:
                                ...
                                l_ProcessingTime = l_Data[l_Server]["Handles"][l_Handle]["Connections"][l_Connection]["LVUuids"][l_LVUuid]["ContribIds"][l_ContribId]["ProcessingTime"]
                                if l_ProcessingTimes == [None,None]:
                                    l_ProcessingTimes = [l_ProcessingTime,l_ProcessingTime]
                                else:
                                    if (l_ProcessingTime < l_ProcessingTimes[0]):
                                        l_ProcessingTimes[0] = l_ProcessingTime
                                    if (l_ProcessingTime > l_ProcessingTimes[1]):
                                        l_ProcessingTimes[1] = l_ProcessingTime

                                if l_HandleProcessingTimes == [None,None]:
                                    l_HandleProcessingTimes = l_ProcessingTimes
                                else:
                                    if (l_ProcessingTimes[0] < l_HandleProcessingTimes[0]):
                                        l_HandleProcessingTimes[0] = l_ProcessingTimes[0]
                                    if (l_ProcessingTimes[1] > l_HandleProcessingTimes[1]):
                                ...
                                if "Files" in l_Data[l_Server]["Handles"][l_Handle]["Connections"][l_Connection]["LVUuids"][l_LVUuid]["ContribIds"][l_ContribId]:
                                    ...
                                    for l_File in l_Files:
                                        l_ReadTime = l_Data[l_Server]["Handles"][l_Handle]["Connections"][l_Connection]["LVUuids"][l_LVUuid]["ContribIds"][l_ContribId]["Files"][l_File]["ReadTime"]
                                        if l_ReadTimes == [None,None]:
                                             l_ReadTimes = [l_ReadTime,l_ReadTime]
                                        else:
                                            if (l_ReadTime < l_ReadTimes[0]):
                                                l_ReadTimes[0] = l_ReadTime
                                            if (l_ReadTime > l_ReadTimes[1]):
                                                l_ReadTimes[1] = l_ReadTime
                                        l_WriteTime = l_Data[l_Server]["Handles"][l_Handle]["Connections"][l_Connection]["LVUuids"][l_LVUuid]["ContribIds"][l_ContribId]["Files"][l_File]["WriteTime"]
                                        if l_WriteTimes == [None,None]:
                                             l_WriteTimes = [l_WriteTime,l_WriteTime]
                                        else:
                                            if (l_WriteTime < l_WriteTimes[0]):
                                                l_WriteTimes[0] = l_WriteTime
                                            if (l_WriteTime > l_WriteTimes[1]):
                    ...
                    # End of connections...
                    l_Output[l_JobId][l_Server]["Handles"]["Connections"]["NumberOfConnections"] = l_NumberOfConnections
            # End of handles...
        ...
        for l_Server in l_Output[l_JobId].keys():
            if type(l_Output[l_JobId][l_Server]) == dict:
                if l_ServerProcessingTimes == [None, None]:
                    ...
                    if (l_Output[l_JobId][l_Server]["Handles"]["ProcessingTimes (ContribId Min/Max)"][0] < l_ServerProcessingTimes[0]):
                        l_ServerProcessingTimes[0] = l_Output[l_JobId][l_Server]["Handles"]["ProcessingTimes (ContribId Min/Max)"][0]
                    if (l_Output[l_JobId][l_Server]["Handles"]["ProcessingTimes (ContribId Min/Max)"][1] > l_ServerProcessingTimes[1]):
...
#        pprint.pprint(l_Output)
        ...
        l_PathFileName = os.path.join(l_Ctx["ROOTDIR"], `l_JobId`, "Details.txt")
