...
def main(*pArgs):
    ...
    for l_Server in l_Servers:
        ...
        for (l_ErrorTime, l_ErrorText) in l_Errors.items():
            l_Output.append("%s:  %s%s" % (l_ErrorTime, l_ErrorText, os.linesep))
        l_Output.append(os.linesep)

    # Output the results
    l_PathFileName = os.path.join(l_Ctx["ROOTDIR"], "Errors.txt")
    cmn.writeOutput(l_Ctx, l_PathFileName, l_Output)
    print "Error results written to %s" % l_PathFileName
