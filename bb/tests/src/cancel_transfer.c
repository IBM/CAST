  ...
  //  LV = argv[1];
    ...
    *************************/
    ...
    if (CancelTransfer) {
       ...
       //printf("Attempting to Cancel Transfer for handle->%lx \n", THandle );
       printf("Attempting to Cancel Transfer for handle->%lld \n", (long long int)THandle );
       rc = BB_CancelTransfer(THandle, BBSCOPETRANSFER);
       check(rc);
       //printf("Successful Cancel Transfer for handle->%lx \n", THandle );
       printf("Successful Cancel Transfer for handle->%lld \n", (long long int)THandle );
...
//    BBTransferDef_t* tdef = pTD->tdef;
 ...
 printf("DEBUG: INSIDE CANCEL Transfer Thread...%d.  Sleeping for 15 seconds\n", (int)pthread_self());
   // sleep(15);                          
    sleep(5);                          
