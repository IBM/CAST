...
// getTransferHandle3.c
//  After genererating a transfer handle for a given BBTAG for a single
//  contributor. retrieve that same transfer handle again. (confirm they are 
//  the same
...
int main(int argc, char** argv)
{
    ...
    rc = BB_GetTransferHandle( getpid(), 1, (uint32_t*)&rank, &thandle1 ); 
    check(rc);
    
    if( rank == 0 )  printf("Obtaining transfer handle a second time\n");
    rc = BB_GetTransferHandle( getpid(), 1, (uint32_t*)&rank, &thandle2 ); 
    check(rc);
