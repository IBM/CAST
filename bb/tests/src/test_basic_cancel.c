...
int main(int argc, char** argv)
{
    ...
    if(rc == -2) rc = 0; // transfer already complete, treating as success
    check(rc);
    MPI_Barrier(MPI_COMM_WORLD);
    stop = MPI_Wtime();
    if(rank == 0)
    {
        printf("PERF(%d,%s,%ld x %d):  BB_CancelTransfer took %g seconds\n", dir, pfspath, filesize, size, stop-start);
        addMetric("bbCancelTransfer_time", stop-start);
    }
    ...
    do
    {
        ...
        check(rc);
        if((info.status == BBFULLSUCCESS) || (info.status == BBPARTIALSUCCESS))
            break;
        sleep(1);
    }
    while(1);
