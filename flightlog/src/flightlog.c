...
int FL_CreateRegistries(const char* rootpath, unsigned int numreg, FlightRecorderCreate_t* flcreate)
{
    int rc;
    unsigned int x;
    char filename[256];
	...
	rc = FL_CreateRegistry(flcreate[x].reg, flcreate[x].name, filename, flcreate[x].decoderName, flcreate[x].size, (FlightRecorderFormatter_t*)flcreate[x].fmt, flcreate[x].numenums);
	if(rc)
	    return rc;
    }
    return 0;
...
int FL_CreateRegistry(FlightRecorderRegistry_t** reghandle, const char* name, const char* filename, const char* decoder, uint64_t length, FlightRecorderFormatter_t* fmt, uint64_t numids)
{
        int rc = 0;
	...
	if((filename != NULL) && (strncmp(filename, "none/",5) != 0))
	{
		...
		fdout = open(filename, O_RDWR | O_CREAT | O_TRUNC | O_CLOEXEC, S_IRWXU);
		if(fdout != -1)
		{
	...
	{
#if __APPLE__
		mapflags = MAP_SHARED | MAP_ANON;
#else
		mapflags = MAP_SHARED | MAP_ANONYMOUS;
#endif
		fdout = -1;
	}
	...
	*reghandle = reg = (FlightRecorderRegistry_t*)mmap(NULL, mmapsize, PROT_READ | PROT_WRITE, mapflags, fdout, 0);
	...
	reg->initEpoch = tv.tv_sec + tv.tv_usec/1000000.0;
	reg->initTimebase = tb;
...
int displayTime(char* buffer, size_t bufferSize, uint64_t timebase, double scale, uint64_t initTimebase, double initEpoch)
{
    if(FL_RAWTIME)
    {
        return snprintf(buffer, bufferSize, "TB=%016llx", (unsigned long long)timebase);
    }
    double wallTime = (double)(timebase - initTimebase) / scale;
...
int FL_Decode(FlightRecorderRegistryList_t* logregistryHead, size_t bufferSize, char* buffer, uint64_t* moreData, uint64_t flags)
{
    ...
    {
        FL_RAWTIME = 1;
        length = snprintf(buffer, bufferSize, "Scale=%g  InitialTimebase=%ld  InitialEpoch=%g\n", logregistryHead->reg->timebaseScale, logregistryHead->reg->initTimebase, logregistryHead->reg->initEpoch);
        bufferSize -= length;
        buffer     += length;
		...
		*moreData = 0;
		...
		if(lsel != NULL)
		{
		    *moreData = lsel->lastOffset + lsel->counttotal + (lsel->id<<48) + 1;
		    ...
		    {
  		        length = displayTime(buffer, bufferSize, log->timestamp, logregistryHead->reg->timebaseScale, logregistryHead->reg->initTimebase, logregistryHead->reg->initEpoch);
                bufferSize -= length;
                buffer     += length;
