...
int main(int argc, char *argv[]) {
    ...
    int32_t l_DataLen = 0, l_NumberOfAttributes = 0, l_NumBytes = 0;
        ...
        txp::Id l_Id = getMsgId();
        {
            ...
            if (!l_RC) {
                ...
                if (!l_RC) {
                    ...
                    // Serialize the message (w/ptrs) into the heap buffer...
                    l_Msg->allocateHeapBuffer();
                    printf("l_Msg->allocateHeapBuffer() %p\n", l_Msg->getHeapBufferPtr());
                    // NOTE:  The data length returned here is NOT the total length of the data to be
                    //        received by the I/O node.  The total length to be sent is the
                    //        length as calculated by getMsgLengthWithDataValues() and is returned
                    //        by build_IO_Vector().
                    l_DataLen = l_Msg->serializeToHeapBuffer();

                    if (l_DataLen>0) {

                        // Build the I/O vector
                        l_Msg->dump(d_log, "Msg, just before build_IO_Vector()");
                        l_DataLen = l_Msg->build_IO_Vector();
                        if (l_DataLen>0) {

                            // Log the send attempt...
                            if (DEBUG > 1) {
                                snprintf(l_LogBuffer, sizeof(l_LogBuffer), "Sending: Msg(%s) with %d attr(s), msg# %d, msglen = %d", l_Temp, l_NumberOfAttributes, l_Msg->getMsgNumber(), l_DataLen);
                                d_log.write(l_LogBuffer, txp::Log::INFO);
                                if (DEBUG > 3) {
                            ...
                            // Send message
                            l_NumBytes = writev(l_FD, l_Msg->get_IO_VectorPtr(), l_Msg->get_IO_VectorCount());
                            if (l_NumBytes < 0) {
                                ...
                                if (l_NumBytes == l_DataLen) {
                                    ...
                                    if (DEBUG) {
                                        snprintf(l_LogBuffer, sizeof(l_LogBuffer), "-> Msg(%s) %d attr(s), msg# %d, bytes/len %d/%zd", l_Temp, l_NumberOfAttributes, l_Msg->getMsgNumber(), l_NumBytes, l_Msg->getSerializedLen());
                                        d_log.write(l_LogBuffer, txp::Log::INFO);
                                    }
                                    ...
                                    while ((!l_RC) && (!l_NumBytes) && l_ReadsWithZeroBytes < l_ReadsWithZeroBytesMax) {
                                        ...
                                        if (l_NumBytes == 0) {
                                            ...
                                            if (l_NumBytes < 0) {
                                                ...
                                                if (DEBUG > 1) {
                                                    snprintf(l_LogBuffer, sizeof(l_LogBuffer), "Received: Response buffer, bytes=%d...", l_NumBytes);
                                                ...
                                                if (!l_RC) {
                                                    ...
                                                    if (txp::Msg::isMsgIdValid(l_Id)) {
                                                        if (txp::Msg::isMsgIdSupported(l_Id)) {
                                                            ...
                                                            if (DEBUG) {
                                                                snprintf(l_LogBuffer, sizeof(l_LogBuffer), "<- Msg(%s), msg# %d to rqst_msg# %d, bytes = %d", l_Temp, l_ResponseMsg->getMsgNumber(), l_ResponseMsg->getRequestMsgNumber(), l_NumBytes);
                                                                d_log.write(l_LogBuffer, txp::Log::INFO);
                                                                if (DEBUG > 3) {
                                    ...
                                    l_RC = -15;
                                    snprintf(l_LogBuffer, sizeof(l_LogBuffer), "-> **ERROR** bytes != datalen: Msg(%s) %d attr(s), msg# %d, bytes/len = %d/%d", l_Temp, l_NumberOfAttributes, l_Msg->getMsgNumber(), l_NumBytes, l_DataLen);
                                    d_log.write(l_LogBuffer, txp::Log::ERROR);
                                }
                            }
                        } else {
                            l_RC = l_DataLen;
                            snprintf(l_LogBuffer, sizeof(l_LogBuffer), "build_IO_Vector() failed, RC=%d", l_RC);
                            d_log.write(l_LogBuffer);
                        }
                    } else {
                        l_RC = (int)l_DataLen;
                        snprintf(l_LogBuffer, sizeof(l_LogBuffer), "serializeToHeapBuffer() failed, RC=%d", l_RC);
                        d_log.write(l_LogBuffer, txp::Log::ERROR);
                ...
                char* l_TempPtr = l_Msg->getDataBufferPtr();
                txp::Log::dump_buffer_raw(d_log, l_TempPtr, 32, "Heap buffer for l_Msg before delete...");
	...
	if (!l_RC) {
    	d_log.write(l_LogBuffer, txp::Log::INFO, txp::Log::DEFAULT_LOGLEVEL, txp::Log::CONSOLE_AND_LOGFILE);
    } else {
    	d_log.write(l_LogBuffer, txp::Log::ERROR, txp::Log::DEFAULT_LOGLEVEL, txp::Log::CONSOLE_AND_LOGFILE);
    }
