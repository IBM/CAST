...
int main(int argc, char** argv) {
	...
	if (!txp::Msg::readConfigFile(l_TxpConfig, l_ConfigFile, sizeof(l_ConfigFile))) {
    	...
    	rc = FL_CreateAll(l_FlightLog.c_str());
    	if (rc) {
			LOG(bb,error) << "Unable to initialize flightlog (" << l_FlightLog << ") rc=" << rc;
			exit(-1);
    	}
    } else {
	...
	l_Msg_4.dump(d_log, "Msg_4 with 19 attributes added, before build");
	...
	l_Msg_4b.dump(d_log, "Msg_4b with 19 copied attributes added");
	...
	l_Msg_4c.dump(d_log, "Msg_4c with 19 directly added attributes");
