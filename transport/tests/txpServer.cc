...
//              }
                ...
                if (l_NumBytes < 0) {
                    ...
                    if (DEBUG > 1) {
                        snprintf(l_LogBuffer, sizeof(l_LogBuffer), "Received: Request buffer, bytes=%zu...", l_NumBytes);
                    ...
                    l_RC = txp::Msg::deserializeToMsg(l_Msg, &l_HeapBuffer, l_DeserializeOption, l_NumBytes);
                    if (!l_RC) {
                        ...
                        if (txp::Msg::isMsgIdValid(l_Id)) {
                            if (txp::Msg::isMsgIdSupported(l_Id)) {
                                ...
                                if (DEBUG > 0) {
                                    snprintf(l_LogBuffer, sizeof(l_LogBuffer), "<- Msg(%s), %d attr(s), msg# %d, bytes/msglen %zu/%zu", l_Temp, l_Msg->getNumberOfAttributes(), l_Msg->getMsgNumber(), l_NumBytes, l_Msg->getMsgLengthWithDataValues());
                                    d_log.write(l_LogBuffer, txp::Log::INFO);
                                }

                    			if (DEBUG > 2) {
                        			l_Msg->dump(d_log, "Received message");
                    			}
...
//                                    printf("l_DataLen=%d\n", l_DataLen);
                        ...
                        snprintf(l_LogBuffer, sizeof(l_LogBuffer), "deserializeToMsg() failed, l_NumBytes = %zu, RC=%d", l_NumBytes, l_RC);
                        d_log.write(l_LogBuffer, txp::Log::ERROR);
                        l_RC = 0;
                    }
                    ...
                    if (!l_RC) {
                        if (!l_txpMsgReceived) {
                            ...
                            if (DEBUG > 1) {
                                snprintf(l_LogBuffer, sizeof(l_LogBuffer), "Sending: text, bytes = %zu", l_DataLen);
                                d_log.write(l_LogBuffer, txp::Log::INFO);
                            }
                        } else {
                            // Log the ensuing send attempt...
                            if (DEBUG > 1) {
                                ...
                                snprintf(l_LogBuffer, sizeof(l_LogBuffer), "Sending: Msg(%s) with %d attr(s), msg# %d, rcv'd bytes/msglen = %zu/%zu", l_Temp, l_ResponseMsg->getNumberOfAttributes(), l_ResponseMsg->getMsgNumber(), l_DataLen, l_ResponseMsg->getSerializedLen());
                                d_log.write(l_LogBuffer, txp::Log::INFO);
                            }
                        }
                    }
           			...
           			if (DEBUG > 2) {
               			l_ResponseMsg->dump(d_log, "Response message");
           			}
                    ...
                    if ((!l_RC) && l_DataLen) {
                        ...
                        if (l_NumBytes <= 0) {
                            snprintf(l_LogBuffer, sizeof(l_LogBuffer), "%s: %zu number of bytes written, msglen %zu, line %d", argv[0], l_NumBytes, l_DataLen, __LINE__);
                            d_log.write(l_LogBuffer, txp::Log::INFO);
                            error(l_LogBuffer);
                        } else {
                            if (l_DataLen == l_NumBytes) {
                                if (DEBUG > 0) {
                                    if (l_txpMsgReceived) {
                                        txp::Msg::msgIdToChar(l_ResponseMsg->getMsgId(), l_Temp, sizeof(l_Temp));
                                        snprintf(l_LogBuffer, sizeof(l_LogBuffer), "-> Msg(%s), %d attr(s), msg# %d, rqst_msg# %d, bytes/msglen %zu/%zu, data=%p, datalen=%zu", l_Temp, l_ResponseMsg->getNumberOfAttributes(), l_ResponseMsg->getMsgNumber(), l_ResponseMsg->getRequestMsgNumber(), l_NumBytes, l_ResponseMsg->getMsgLengthWithDataValues(), l_Data, l_DataLen);
                                    } else {
                                ...
                                if (l_txpMsgReceived) {
                                    txp::Msg::msgIdToChar(l_ResponseMsg->getMsgId(), l_Temp, sizeof(l_Temp));
                                    snprintf(l_LogBuffer, sizeof(l_LogBuffer), "-> **ERROR** bytes != datalen:  Msg(%s), %d attr(s), msg# %d, rqst_msg# %d, bytes/msglen %zu/%zu, data=%p, datalen=%zu", l_Temp, l_ResponseMsg->getNumberOfAttributes(), l_ResponseMsg->getMsgNumber(), l_ResponseMsg->getRequestMsgNumber(), l_NumBytes, l_ResponseMsg->getMsgLengthWithDataValues(), l_Data, l_DataLen);
                                } else {
                                    snprintf(l_LogBuffer, sizeof(l_LogBuffer), "-> **ERROR** bytes != datalen:  data=%p, bytes %zu, datalen=%zu", l_Data, l_NumBytes, l_DataLen);
                                }
                                d_log.write(l_LogBuffer, txp::Log::ERROR);
                                l_RC = -14;
                            }
