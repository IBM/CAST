...
const int HIGH_NUMBER_OF_ATTRIBUTES = 24;
//const int HIGH_NUMBER_OF_ATTRIBUTES = 192;

//const int LOW_NUMBER_OF_CHAR_ARRAYS = 0;
const int LOW_NUMBER_OF_CHAR_ARRAYS = 1;

//const int HIGH_NUMBER_OF_CHAR_ARRAYS = 1;
const int HIGH_NUMBER_OF_CHAR_ARRAYS = 16;

const int LOW_CHAR_LENGTH = 1;
const int HIGH_CHAR_LENGTH = 64;
//const int HIGH_CHAR_LENGTH = 256;
//const int HIGH_CHAR_LENGTH = 512;
...
int32_t getNumberOfElements(unsigned int pValue) {
    return ((rand() % (HIGH_NUMBER_OF_CHAR_ARRAYS - LOW_NUMBER_OF_CHAR_ARRAYS + 1)) + LOW_NUMBER_OF_CHAR_ARRAYS);
}
...
int main(int argc, char *argv[]) {
    ...
    int32_t l_NumberOfAttributes = 0, l_NumberOfElements = 0;
    ssize_t l_DataLen = 0, l_NumBytes = 0;
    char* l_Data = 0;

    txp::Msg* l_Msg = 0;
    txp::Msg* l_ResponseMsg = 0;
//    txp::AttrPtr_array_of_char_arrays* l_Attr = 0;
    txp::CharArray* l_CharArray = 0;
    while ((!l_RC) && ((!l_NumberOfMsgs) || l_NumberOfMsgsSent<l_NumberOfMsgs)) {
        ...
        txp::Id l_Id = getMsgId();
        {
            ...
            if (!l_RC) {
                ...
                for (int i=0; i<l_NumberOfAttributes && l_RC==0; ++i) {
                    l_CharArray = new txp::CharArray();
                    {
                    	txp::CharArray_Element l_CharArrayElement = txp::CharArray_Element(getCharLength(rand()), TestData);
 	                	l_NumberOfElements = getNumberOfElements(rand());
                    	for (int j=0; j<l_NumberOfElements && l_RC==0; ++j) {
                    		l_CharArray->push_back(l_CharArrayElement);
                    	}
/*
                    	l_RC = txp::AttrPtr_array_of_char_arrays::buildAttrPtr_array_of_char_arrays(getAttrName(i), l_CharArray, l_Attr);
                    	if (!l_RC) {
//                    		l_Attr->dump(d_log, "Original attribute");
//                    	    txp::AttrPtr_array_of_char_arrays* l_Attr2 = l_Attr;
//                    	    l_Attr2->dump(d_log, "Copied attribute");
...
*/
                    	l_RC = l_Msg->addAttribute(getAttrName(i), l_CharArray);
                    	if (l_RC == -30) {
                    		// Duplicate attribute name...
                    		l_RC = 0;
                    		--i;
                ...
                if (!l_RC) {
                    ...
                    if (l_DataLen>0) {
...

                        {
                            ...
                            if (DEBUG > 1) {
                                snprintf(l_LogBuffer, sizeof(l_LogBuffer), "Sending: Msg(%s) with %d attr(s), msg# %d, bytes/msglen = %zu/%zd", l_Temp, l_NumberOfAttributes, l_Msg->getMsgNumber(), l_DataLen, l_Msg->getSerializedLen());
                                d_log.write(l_LogBuffer, txp::Log::INFO);
                                if (DEBUG > 3) {
