...
// NOTE:  THIS CLIENT TEST PROGRAM IS NOW BROKEN, AS YOU CANNOT ADD THE SAME NAMED
//        ATTRIBUTE TO A GIVEN Msg() MULTIPLE TIME.  NOT SURE IF WE WANT TO FIX...  @@DLH
...
int main(int argc, char *argv[]) {
    ...
    while ((!l_RC) && ((!l_NumberOfMsgs) || l_NumberOfMsgsSent<l_NumberOfMsgs)) {
        ...
        {
            {
            	// Create the message (on the stack...)
            	txp::Msg l_Msg(l_Id);

                // Create the attribute (on the stack...)
                txp::AttrPtr_char_array l_Attr(getAttrName(rand()), TestData, getCharLength(rand()));
                ...
                if (!l_RC) {
                    ...
                    if (l_DataLen>0) {
                        ...
                        l_Data = l_Msg.getDataBufferPtr();
                        {
                            ...
                            if (DEBUG > 1) {
                                snprintf(l_LogBuffer, sizeof(l_LogBuffer), "Sending: Msg(%s) with %d attr(s), msg# %d, bytes/msglen = %d/%zd", l_Temp, l_NumberOfAttributes, l_Msg.getMsgNumber(), l_DataLen, l_Msg.getSerializedLen());
                                d_log.write(l_LogBuffer, txp::Log::INFO);
                                if (DEBUG > 3) {
