/*================================================================================
    ...
    csmi/src/bb/tests/test_csmi_bb_lv_update_2.c
...
/*
* Program to test the csm_bb_lv_update CSM API. 
*/
int csmi_client(int argc, char *argv[])
{
	csmutil_logging(trace, "%s-%d:", __FILE__, __LINE__);
	csmutil_logging(trace, "  Entered test_csmi_bb_lv_update.");
	...
	/*Input the values we want for our test. */
	input->allocation_id = 1;
	input->current_size = 128;
	...
	puts("############");
	puts("#End code inside 'csmi_client' of 'test_csmi_bb_lv_update.c'");
	puts("############");
	puts("#");
	puts("#");
	puts("#");
