/*================================================================================
    ...
    csmi/src/bb/tests/test_csmi_bb_lv_delete.c
...
/*
* Program to test the csm_bb_lv_delete CSM API. 
*/
int csmi_client(int argc, char *argv[])
{
	csmutil_logging(trace, "%s-%d:", __FILE__, __LINE__);
	csmutil_logging(trace, "  Entered test_csmi_bb_lv_delete.");
	...
	/*Call the API*/
	returnValue = csm_bb_lv_delete(&csm_obj, input);
	...
	if(returnValue != 0){
		...
		csmutil_logging(debug, "%s-%d:", __FILE__, __LINE__);
		csmutil_logging(debug, "  csm_bb_lv_delete has completed successfully!");
	}else{
		csmutil_logging(error, "%s-%d:", __FILE__, __LINE__);
	...
	puts("############");
	puts("#End code inside 'csmi_client' of 'test_csmi_bb_lv_delete.c'");
	puts("############");
	puts("#");
	puts("#");
	puts("#");
