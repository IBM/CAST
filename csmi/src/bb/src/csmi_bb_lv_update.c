/*================================================================================
    ...
    csmi/src/bb/src/csmi_bb_lv_update.c
...
int csm_bb_lv_update(csm_api_object **csm_obj, API_PARAMETER_INPUT_TYPE* input)
{
                ...
                || input->state == 'R' ) )
    {
        ...
        {
           csmutil_logging(error, "Invalid parameter: 'allocation_id' must be positive."); 
        }
        else if ( input->current_size < 0 )
        {
           csmutil_logging(error, "Invalid parameter: 'current_size' must be positive."); 
        }
        else if ( input->logical_volume_name == NULL || input->logical_volume_name[0] == '\0' )
        {
           csmutil_logging(error, "Invalid parameter: 'logical_volume_name' must be specified."); 
        }
		else if ( input->node_name == NULL || input->node_name[0] == '\0' )
        {
           csmutil_logging(error, "Invalid parameter: 'node_name' must be specified."); 
        }
        else
        {
		    csmutil_logging(error, "Invalid parameter: 'state' must be 'C','M','S', or 'R'");
        }
