/*================================================================================
    ...
    csmi/src/wm/tests/test_csmi_allocation_step_query_details.c
...
#if 0
void release_args(csm_allocation_step_query_details_args_t* arguments)
{
	if(arguments){
...
void release_result(csm_allocation_step_query_details_result_t* resultRecord)
{
	csmutil_logging(trace, "%s-%d:", __FILE__, __LINE__);
	csmutil_logging(trace, "  inside release_result()");
	if(resultRecord){
		csmutil_logging(trace, "%s-%d:", __FILE__, __LINE__);
		csmutil_logging(trace, "  resultRecord true");
		...
		if(resultRecord->node_names){
			uint32_t i = 0;
			for(i = 0; i < resultRecord->num_nodes; i++){
				if(resultRecord->node_names[i]){free(resultRecord->node_names[i]);}
			}
			free(resultRecord->node_names);
		}
		free(resultRecord);
		csmutil_logging(debug, "%s-%d:", __FILE__, __LINE__);
		csmutil_logging(debug, "  resultRecord successfully freed");
	}
	csmutil_logging(trace, "%s-%d:", __FILE__, __LINE__);
	csmutil_logging(trace, "  leaving release_result()");
}
#endif
...
int csmi_client(int argc, char *argv[])
{
	...
	puts("############");
	puts("#Begin code inside 'csmi_client' of 'test_csmi_allocation_step_query_details.c'");
	...
	/*test data*/
	csm_allocation_step_query_details_args_t* arguments = NULL;
	arguments = malloc(sizeof(csm_allocation_step_query_details_args_t));
	//arguments->step_id = "1";
	...
	csm_allocation_step_query_details_result_t **results = NULL;
	...
	returnValue = csm_allocation_step_query_details(&csm_obj, arguments, &results, &dataCount);
	...
	puts("#####");
	puts("#called csm_allocation_step_query_details");
	...
	if(dataCount > 0){
		for(i = 0; i < dataCount; i++){
			...
			printf("  node_names:\n");
			uint32_t j = 0;
			for(j = 0; j < results[i]->num_nodes; j++){
				printf("    node_names[%u]: %s\n", j, results[i]->node_names[j]);
			}
		}
	}else if(dataCount == 0){
	...
	puts("############");
	puts("#End code inside 'csmi_client' of 'test_csmi_allocation_step_query_details.c'");
	puts("############");
#endif	
	return 0;
}
