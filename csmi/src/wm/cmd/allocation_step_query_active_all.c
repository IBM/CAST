/*================================================================================
    ...
    csmi/src/wm/cmd/allocation_step_query_active_all.c
...
#define USAGE csm_free_struct(csm_allocation_step_query_active_all_input_t, input); help


void _notes_release()
{
	csmutil_logging(trace, "%s-%d: Inside of release", __FILE__, __LINE__);
	csmutil_logging(trace, "%s-%d: Checking to see if the pointer passed in is NULL or points to a csmi_diag_run_t struct.", __FILE__, __LINE__);
	csmutil_logging(trace,    "%s-%d: Leaving release", __FILE__, __LINE__);
}
...
void help() {
	puts("_____CSM_ALLOCATION_STEP_QUERY_ACTIVE_ALL_CMD_HELP_____");
	puts("USAGE:");
	puts("  csm_allocation_step_query_active_all ARGUMENTS [OPTIONS]");
	puts("  csm_allocation_step_query_active_all -a allocation_id [-h] [-v verbose_level]");
	puts("");
	puts("SUMMARY: Used to gather information about all active steps for an allocation from the CSM database.");
	puts("");
	puts("EXIT STATUS:");
	puts("  0  if OK,");
	puts("  1  if ERROR.");
	...
	puts("ARGUMENTS:");
	puts("  MANDATORY:");
	puts("    csm_allocation_step_query_active_all expects 1 mandatory argument");
	puts("    Argument                | Example value      | Description  ");                                                 
	puts("    ------------------------|--------------------|--------------");
	puts("    -a, --allocation_id     | 1                  | (STRING) Primary job id (for lsf this will be the lsf job id).");
	puts("                            |                    | ");
	puts("");
	...
	puts("  csm_allocation_step_query_active_all -a 1");
	puts("");
...
/*
* Summary: Simple command line interface for the CSM API 'allocation step query active all'. 
...
int main(int argc, char *argv[])
{
	...
	if( return_value != 0){
		csmutil_logging(error, "%s-%d:", __FILE__, __LINE__);
		csmutil_logging(error, "  csm_init_lib rc= %d, Initialization failed. Success is required to be able to communicate between library and daemon. Are the daemons running?", 
                            return_value);
	...
	return_value = csm_allocation_step_query_active_all(&csm_obj, &input, &output);
    csm_free_struct(API_PARAMETER_INPUT_TYPE, input);
