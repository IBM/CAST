/*================================================================================
    ...
    csmi/src/inv/src/csmi_node_delete.c
    ...
    API_PARAMETER_OUTPUT_TYPE** output )
{
    ...
    create_csm_api_object(csm_obj, expected_cmd, csmi_node_delete_destroy);
    
    // Check the input structure.
    if( !input || 
            input->node_names_count <= 0 || 
			input->node_names == NULL  || input->node_names[0] == NULL || input->node_names[0][0] == '\0' 
        )

    {
        ...
        {
            csmutil_logging(error, "Invalid parameter: 'node_names_count' "
                " must be greater than zero.");
        }
        else if ( input->node_names == NULL || input->node_names[0] == NULL || input->node_names[0][0] == '\0')
        {
            csmutil_logging(error, "Invalid parameter: 'node_names' can not be NULL.");
        }
        else
        {
            csmutil_logging(error, "default: Invalid parameter.");
        }
    ...
    if ( error_code == CSMI_SUCCESS )
    {
                ...
                return_buffer, return_buffer_length ) == 0 )
        {
            ...
            {
                csmutil_logging(error, "Could not delete %d node(s)", (*output)->failure_count );
                
                csm_api_object_errcode_set(*csm_obj, CSMERR_DEL_MISMATCH);
                csm_api_object_errmsg_set(*csm_obj,
                    strdup(csm_get_string_from_enum(csmi_cmd_err_t, CSMERR_DEL_MISMATCH)));
                error_code = CSMERR_DEL_MISMATCH;
            }
        }
        else
        {
