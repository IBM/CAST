/*================================================================================
    ...
    csmi/src/inv/src/csmi_switch_attributes_update.c
...
int csm_switch_attributes_update(
    ...
    API_PARAMETER_OUTPUT_TYPE* *output)
{
    ...
    create_csm_api_object(csm_obj, expected_cmd, csmi_switch_attributes_update_destroy);
    
    // Check the input structure.
    if( !input || 
            input->switch_names_count  <= 0 || input->switch_names == NULL )
    {
        if ( !input )
        {
            csmutil_logging(error, "Invalid parameter: input parameter was null." );
        }
        else if ( input->switch_names_count  <= 0 || input->switch_names == NULL )
        {
            csmutil_logging(error, "Invalid parameter: 'switch_names_count' and 'switch_names'"
                " must specify one or more switch.");
        }
        else
        {
    ...
    if ( error_code == CSMI_SUCCESS )
    {
                ...
                return_buffer, return_buffer_length ) == 0 )
        {
            ...
            {
                csmutil_logging(error, "Could not update %d switch(es)", 
...
void csmi_switch_attributes_update_destroy(csm_api_object *csm_obj)
{
    ...
    output = (csm_switch_attributes_update_output_t *) csmi_hdl->ret_cdata;
    csm_free_struct_ptr( csm_switch_attributes_update_output_t, output );
}
