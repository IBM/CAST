/*================================================================================
    ...
    csmi/src/inv/src/csmi_node_attributes_query_history.c
...
int csm_node_attributes_query_history(
    ...
    API_PARAMETER_OUTPUT_TYPE** output)
{
    ...
    create_csm_api_object(csm_obj, expected_cmd, csmi_node_attributes_query_history_destroy);

    // EARLY RETURN
    // The node_name must not be NULL
    if( !input || input->node_name == NULL)
    {
        csmutil_logging(error, "Invalid parameter: 'node_name' can not be NULL");
