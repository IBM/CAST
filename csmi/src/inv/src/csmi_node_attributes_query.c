/*================================================================================
    ...
    csmi/src/inv/src/csmi_node_attributes_query.c
    ...
    API_PARAMETER_OUTPUT_TYPE** output)
{
    ...
    create_csm_api_object(csm_obj, expected_cmd, csmi_node_attributes_query_destroy);

    // EARLY RETURN
    // The input must not be NULL
    if( !input 
		|| (input->comment == NULL && input->node_names_count <= 0 && input->state == CSM_NODE_NO_DEF && input->type == CSM_NODE_NO_TYPE) //Missing search criteria. 
		|| ( input->state < CSM_NODE_NO_DEF || input->state >=csm_enum_max(csmi_node_state_t)) //outside the state bounds
		|| ( input->type < CSM_NODE_NO_TYPE || input->type >=csm_enum_max(csmi_node_type_t)) //outside the type bounds
		|| ( input->comment != NULL && input->comment[0] == '\0') // comment allocated but is NULL
	)
    {
		if( !input )
		{
		...
		else if(input->comment == NULL && input->node_names_count <= 0 && input->state == CSM_NODE_NO_DEF && input->type == CSM_NODE_NO_TYPE)
		{	
		...
		else if( input->state < CSM_NODE_NO_DEF || input->state >=csm_enum_max(csmi_node_state_t))
		{
			//outside the state enum bounds
			csmutil_logging(error, "%s-%d:", __FILE__, __LINE__);
			csmutil_logging(error, "  Invalid parameter: 'input->state' outside of state enum bounds.");
			csm_api_object_errcode_set(*csm_obj, CSMERR_INVALID_PARAM);
			csm_api_object_errmsg_set(*csm_obj, strdup(csm_get_string_from_enum(csmi_cmd_err_t, CSMERR_INVALID_PARAM)));
			return CSMERR_INVALID_PARAM;
		}
		else if( input->type < CSM_NODE_NO_TYPE || input->type >=csm_enum_max(csmi_node_type_t))
		{
			//outside the type enum bounds
			csmutil_logging(error, "%s-%d:", __FILE__, __LINE__);
			csmutil_logging(error, "  Invalid parameter: 'input->type' outside of type enum bounds.");
			csm_api_object_errcode_set(*csm_obj, CSMERR_INVALID_PARAM);
			csm_api_object_errmsg_set(*csm_obj, strdup(csm_get_string_from_enum(csmi_cmd_err_t, CSMERR_INVALID_PARAM)));
			return CSMERR_INVALID_PARAM;
		}
		else if ( input->comment != NULL && input->comment[0] == '\0')
		{
			// comment allocated but is NULL
			csmutil_logging(error, "%s-%d:", __FILE__, __LINE__);
			csmutil_logging(error, "  Invalid parameter: 'input->comment' if allocated, then can not be NULL.");
			csm_api_object_errcode_set(*csm_obj, CSMERR_INVALID_PARAM);
			csm_api_object_errmsg_set(*csm_obj, strdup(csm_get_string_from_enum(csmi_cmd_err_t, CSMERR_INVALID_PARAM)));
			return CSMERR_INVALID_PARAM;
		}
