/*================================================================================
    ...
    csmi/src/inv/src/csmi_switch_children_inventory_collection_temp.c
...
int csm_switch_children_inventory_collection(
    ...
    API_PARAMETER_OUTPUT_TYPE* *output)
{
    ...
    create_csm_api_object(csm_obj, expected_cmd, csmi_switch_children_inventory_collection_destroy);
    
    if( !input )
    {
        csmutil_logging(error, "Invalid parameter: Must not be NULL.");
