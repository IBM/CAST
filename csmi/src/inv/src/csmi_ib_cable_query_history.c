/*================================================================================
    ...
    csmi/src/inv/src/csmi_ib_cable_query_history.c
...
int csm_ib_cable_query_history(csm_api_object **csm_obj, API_PARAMETER_INPUT_TYPE* input, API_PARAMETER_OUTPUT_TYPE* *output)
{
    START_TIMING()
    ...
    create_csm_api_object(csm_obj, expected_cmd, csmi_ib_cable_query_history_destroy);

    if( !input || input->serial_number == NULL )
    {
        csmutil_logging(error, "Invalid parameter: 'serial_number' must specify a serial number");
