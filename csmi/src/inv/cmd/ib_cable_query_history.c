/*================================================================================
    ...
    csmi/src/inv/cmd/ib_cable_query_history.c
...
void help(){
	puts("_____CSM_IB_CABLE_QUERY_HISTORY_CMD_HELP_____");
	puts("USAGE:");
	puts("  csm_ib_cable_query_history ARGUMENTS [OPTIONS]");
	puts("  csm_ib_cable_query_history [-s serial_number] [-l limit] [-o offset] [-h] [-v verbose_level]");
	puts("");
	puts("SUMMARY: Used to query the 'csm_ib_cable_history' table of the CSM database.");
	puts("");
	puts("EXIT STATUS:");
	puts("  0  if OK,");
	puts("  1  if ERROR.");
	...
	puts("ARGUMENTS:");
	puts("  MANDATORY:");
	puts("    csm_ib_cable_query_history has 1 mandatory argument");
	puts("    Argument            | Example value | Description  ");                                                 
	puts("    --------------------|---------------|--------------");
	puts("    -s, --serial_number | \"abc123\"      | (STRING) This is the serial number to query. Filter results to only include records that have a matching serial number. The serial number is a unique identification for an ib cable.");
	puts("                        |               | ");
	puts("  FILTERS:");
	puts("    csm_ib_cable_query_history can have 2 optional filters.");
	puts("    Argument     | Example value | Description  ");                                                 
	puts("    -------------|---------------|--------------");
	...
	puts("  csm_ib_cable_query_history -s \"abc123\"");
...
{	
	...
	csmutil_logging(debug, "    serial_number:       %s", input->serial_number);
	csmutil_logging(debug, "  value of output:        %p", output);
	csmutil_logging(debug, "  address of output:      %p", &output);
	
	/* Call the actual CSM API */
	return_value = csm_ib_cable_query_history(&csm_obj, input, &output);
