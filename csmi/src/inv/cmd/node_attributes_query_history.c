/*================================================================================
    ...
    csmi/src/inv/cmd/node_attributes_query_history.c
...
void help(){
	puts("_____CSM_NODE_ATTRIBUTES_QUERY_HISTORY_CMD_HELP_____");
	puts("USAGE:");
	puts("  csm_node_attributes_query_history ARGUMENTS [OPTIONS]");
	puts("  csm_node_attributes_query_history -n node_name [-l limit] [-o offset] [-O order_by] [-h] [-v verbose_level]");
	puts("");
	puts("SUMMARY: Used to query a record in the 'csm_node_history' table of the csm database.");
	puts("");
	puts("EXIT STATUS:");
	puts("  0  if OK,");
	puts("  1  if ERROR.");
	...
	puts("ARGUMENTS:");
	puts("  MANDATORY:");
	puts("    csm_node_attributes_query_history expects 1 mandatory parameter");
	puts("    Argument        | Example value | Description  ");                                                 
	puts("    ----------------|---------------|--------------");
	puts("    -n, --node_name | \"node01\"      | (STRING) Identifies which node this information is for.");
	puts("");
	puts("  FILTERS:");
	puts("    csm_node_attributes_query_history can have 3 optional filters.");
	puts("    Argument       | Example value | Description  ");                                                 
	puts("    ---------------|---------------|--------------");
	...
	puts("  csm_node_attributes_query_history -n \"node01\"");
	puts("");
...
{	
	...
	/*check optional args*/
	while ((opt = getopt_long(argc, argv, "hv:l:n:o:O:", longopts, &indexptr)) != -1) {
		switch(opt){
			case 'h':
                USAGE();
				return CSMI_HELP;
	...
	return_value = csm_node_attributes_query_history(&csm_obj, input, &output);
