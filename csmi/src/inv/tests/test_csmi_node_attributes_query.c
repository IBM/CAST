/*================================================================================
    ...
    csmi/src/inv/tests/test_csmi_node_attributes_query.c
...
{	
	...
	csmutil_logging(debug, "====================");
	csmutil_logging(debug, "  Test 0: Input MAIN");
	csmutil_logging(debug, "  \n\n\n");
	...
	csmutil_logging(debug, "  \n\n\n");
	csmutil_logging(debug, "====================");
	csmutil_logging(debug, "  Test 1: Input NULL");
	csmutil_logging(debug, "  using: Input 2");
	csmutil_logging(debug, "  \n\n\n");
	
	API_PARAMETER_INPUT_TYPE* input2 = NULL;

	//This will print out the contents of the struct that we will pass to the api
	csmutil_logging(debug, "%s-%d:", __FILE__, __LINE__);
	csmutil_logging(debug, "  Preparing to call the CSM API...");
	csmutil_logging(debug, "  value of input:    %p", input2);
	if(input2 != NULL)
	{
		csmutil_logging(debug, "  address of input:  %p", &input2);
		csmutil_logging(debug, "  input contains the following:");
		csmutil_logging(debug, "    comment:          %s", input2->comment);
		csmutil_logging(debug, "    limit:            %i", input2->limit);
		csmutil_logging(debug, "    node_names_count: %i", input2->node_names_count);
		csmutil_logging(debug, "    node_names:       %p", input2->node_names);
		for(i = 0; i < input2->node_names_count; i++){
			csmutil_logging(debug, "      node_names[%i]: %s", i, input2->node_names[i]);
		}
		csmutil_logging(debug, "    offset:           %i", input2->offset);
		csmutil_logging(debug, "    state:            %s", csm_get_string_from_enum(csmi_node_state_t, input2->state) );
		csmutil_logging(debug, "    type:             %s", csm_get_string_from_enum(csmi_node_type_t, input2->type) );
	}
	
	csmutil_logging(debug, "  calling the CSM API...");
    /* Call the C API. */
	return_value = csm_node_attributes_query(&csm_obj, input2, &output);
	...
	//TEST 2 - Input initialized but no vars set
	csmutil_logging(debug, "  \n\n\n");
	csmutil_logging(debug, "====================");
	csmutil_logging(debug, "  Test 2: Input initialized but no vars set");
	csmutil_logging(debug, "  using: Input 2");
	csmutil_logging(debug, "  \n\n\n");
	
	csm_init_struct_ptr(API_PARAMETER_INPUT_TYPE, input2);
	...
	csmutil_logging(debug, "  \n\n\n");
	csmutil_logging(debug, "====================");
	csmutil_logging(debug, "  Test 3: bad state");
	csmutil_logging(debug, "  using: Input 2");
	csmutil_logging(debug, "  \n\n\n");
	...
	//TEST 4 - another bad state
	csmutil_logging(debug, "  \n\n\n");
	csmutil_logging(debug, "====================");
	csmutil_logging(debug, "  Test 4: another bad state");
	csmutil_logging(debug, "  using: Input 2");
	csmutil_logging(debug, "  \n\n\n");
	...
	//TEST 5 - comment allocated but is null char
	csmutil_logging(debug, "  \n\n\n");
	csmutil_logging(debug, "====================");
	csmutil_logging(debug, "  Test 5: comment allocated but is null char");
	csmutil_logging(debug, "  using: Input 2");
	csmutil_logging(debug, "  \n\n\n");
	
	input2->state = CSM_NODE_NO_DEF;
	input2->comment = calloc(1, sizeof(char));
	input2->comment[0] = '\0';
	...
	//clear old tests values
		input2->comment = NULL;
	free(input2->comment);
	
	//TEST 6 - node names count > 0 but first value is NULL
	csmutil_logging(debug, "  \n\n\n");
	csmutil_logging(debug, "====================");
	csmutil_logging(debug, "  Test 6: node names count > 0 but first value is NULL");
	csmutil_logging(debug, "  using: Input 2");
	csmutil_logging(debug, "  \n\n\n");
	
	// database doesnt crash on this guy. so maybe we don't need to check for this test case. 

	
	input2->node_names_count = 1;
	input2->node_names = calloc(input2->node_names_count, sizeof(char*));
