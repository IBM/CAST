/*================================================================================
    ...
    csmi/src/diag/src/csmi_diag_run_query.c
    ...
    API_PARAMETER_OUTPUT_TYPE **output)
{
    ...
    create_csm_api_object(csm_obj, expected_cmd, csmi_diag_run_query_destroy);
    ...
    if ( error_code == CSMI_SUCCESS )
    {
        if ( return_buffer )
        {
            if ( csm_deserialize_struct(API_PARAMETER_OUTPUT_TYPE, output,
                    (const char*)return_buffer, return_buffer_length) == 0 )
            {
...
void csmi_diag_run_query_destroy(csm_api_object *csm_obj)
{
    ...
    {
        csmutil_logging(warning, "%s-%d: csm_api_object not valid", 
            __FILE__, __LINE__)    ;
        return;
    }
