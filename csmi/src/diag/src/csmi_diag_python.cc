...
tuple wrap_csm_diag_result_create(
    csm_diag_result_create_input_t input)
{
    ...
    // Run the API
    int return_code = csm_diag_result_create( (csm_api_object**)&updated_handle, &input);
    int64_t oid = CSMIObj::GetInstance().StoreCSMObj(updated_handle);

    return make_tuple(return_code, oid);
}

tuple wrap_csm_diag_run_begin(
    csm_diag_run_begin_input_t input)
{
    // Always sets the metadata.
    input._metadata=CSM_VERSION_ID;
    ...
    // Run the API
    int return_code = csm_diag_run_begin( (csm_api_object**)&updated_handle, &input);
    int64_t oid = CSMIObj::GetInstance().StoreCSMObj(updated_handle);

    return make_tuple(return_code, oid);
}

tuple wrap_csm_diag_run_end(
    csm_diag_run_end_input_t input)
{
    // Always sets the metadata.
    input._metadata=CSM_VERSION_ID;
    ...
    // Run the API
    int return_code = csm_diag_run_end( (csm_api_object**)&updated_handle, &input);
    int64_t oid = CSMIObj::GetInstance().StoreCSMObj(updated_handle);

    return make_tuple(return_code, oid);
}

tuple wrap_csm_diag_run_query(
    csm_diag_run_query_input_t input)
{
    // Always sets the metadata.
    input._metadata=CSM_VERSION_ID;
    ...
    // Output objs
    csm_api_object * updated_handle;
    csm_diag_run_query_output_t * output= nullptr;

    // Run the API
    int return_code = csm_diag_run_query( (csm_api_object**)&updated_handle, &input, &output);
    int64_t oid = CSMIObj::GetInstance().StoreCSMObj(updated_handle);

    return make_tuple(return_code, oid, *output);
}

tuple wrap_csm_diag_run_query_details(
    csm_diag_run_query_details_input_t input)
{
    ...
    // Output objs
    csm_api_object * updated_handle;
    csm_diag_run_query_details_output_t * output= nullptr;

    // Run the API
    int return_code = csm_diag_run_query_details( (csm_api_object**)&updated_handle, &input, &output);
    int64_t oid = CSMIObj::GetInstance().StoreCSMObj(updated_handle);
...
{
    // Diagnostics
    def("diag_result_create",
        wrap_csm_diag_result_create,
        CSM_GEN_DOCSTRING("Creates a diagnostic result in the CSM database.",
            ""));

    def("diag_run_begin",
        wrap_csm_diag_run_begin,
        CSM_GEN_DOCSTRING("Records the start of a diagnostic run in the CSM database.",
            ""));

    def("diag_run_end",
        wrap_csm_diag_run_end,
        CSM_GEN_DOCSTRING("Records the end of a diagnostic run in the CSM database.",
            ""));

    def("diag_run_query",
        wrap_csm_diag_run_query,
        CSM_GEN_DOCSTRING("Performs a query for diagnostic runs in the CSM database.",
            ",csm_diag_run_query_output_t"));

    def("diag_run_query_details",
        wrap_csm_diag_run_query_details,
        CSM_GEN_DOCSTRING("Performs a details query for diagnostic runs in the CSM database.",
